...
class BERTNER(BertPreTrainedModel):
    ...
    '''
    def __init__(self, model_file, classes, scheme, seed=None, num_heads=2):
        '''
    ...
    def build_model(self):
        ...
        self.conv1d = nn.Conv1d(self.config.hidden_size, self.config.hidden_size, kernel_size=3, padding=1)
        # max pool layer
        self.max_pool = nn.MaxPool1d(kernel_size=2, stride=2)
        # dense classification layer
        self.classifier = nn.Linear(self.config.hidden_size, len(self.classes))
        # multi-head attention layer
        self.multihead_attn = nn.MultiheadAttention(self.config.hidden_size,self.num_heads)
        # CRF output layer
    ...
    #  lstm_output = lstm_output[:, :-1, :]
     cnn_output = self.conv1d(sequence_output.transpose(1, 2)).transpose(1, 2)
     cnn_output = F.relu(cnn_output)
     
    #  final_output = torch.cat((lstm_output, cnn_output[:, :, self.config.hidden_size//2:]), dim=2)

     # multi-head attention
     multihead_output = self.multihead_attn(cnn_output)
     multihead_output = self.dropout(multihead_output)
     # classification logits
     logits = self.classifier(multihead_output)
